...
# ==============================================================================
[[ -n "${SLURM_ARRAY_JOB_ID:-}"  ]] || die "SLURM_ARRAY_JOB_ID not set"
...
JOB_FILE="${JOB_PROVENANCE_DIR}/job.txt"               # once per job
SUBMIT_FILE="${JOB_PROVENANCE_DIR}/script.sh"          # once per job

TASK_FILE="${TASK_PROVENANCE_DIR}/task.txt"             # per task
RUN_FILE="${TASK_PROVENANCE_DIR}/run.txt"              # per task
ENV_FILE="${TASK_PROVENANCE_DIR}/env.txt"              # per task

STATUS_FILE="${RUN_DIR}/STATUS"

# lock to ensure "once per job" writers only run once
...
# ==============================================================================
log STEP "capture execution code (once per job)"
# ==============================================================================
if ( set -o noclobber; : >"${JOB_LOCK}" ) 2>/dev/null; then 
  ...
  # ==============================================================================
  log STEP "capture job-level provenance"
  # ==============================================================================
